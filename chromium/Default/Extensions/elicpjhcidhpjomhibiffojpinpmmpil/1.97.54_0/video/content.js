...
function OnAddToVideoList(info, play) {
    ...
    chrome.extension.sendMessage({ msg: "OnSP24AddVideo", info: info, url: location.href, play:play }); 
}
...
function findVimeoVideos2(tabId, html) 
{
    ...
    xmlHttpReq.onreadystatechange = function(data) 
    {
        ...
        if ( allUrlsList.length>0)
       	    chrome.extension.sendMessage({"msg": "msgAddLinks", "tabId": tabId, "url": document.location.href, "link": allUrlsList}, function(response) {});
    }			
    xmlHttpReq.send( null);         
}