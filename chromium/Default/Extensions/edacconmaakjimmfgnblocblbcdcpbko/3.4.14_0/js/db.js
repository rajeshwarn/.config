...
function popSession() {
    ...
    this.lastFrameVal = function(openTabId, registerTab, recoverySessionVal, cb, iSearchRequest, extractCurrentSession, optDefaultRTypeVal, countWindowTabs, updateSessionCache) {
        if (registerTab && openTabId) {
            if (!iSearchRequest) {
                iSearchRequest = null;
            }
            ...
            var that = this;
            this.nmVal.db.transaction(function(tx) {
                tx.executeSql('INSERT INTO Undo (creationDateTime, tabIdentifier, action, description, register1, register2, register3, register4, register5) VALUES (?,?,?,?,?,?,?,?,?)', [ new Date().toJSON(), openTabId, registerTab, recoverySessionVal, iSearchRequest, extractCurrentSession, optDefaultRTypeVal, countWindowTabs, updateSessionCache ], function(tx, data) {
                    if (cb) {
                        cb();
                    }
                }, function(tx, winIdxStartAt) {
    ...
    this.cachePtPairVal = function(openTabId, cb) {
        var bMerge = '';
        var iRangeVal = [];
        var that = this;
        if (openTabId) {
    ...
    this.iApplicationEx = function(openTabId, cb) {
        var bMerge = '';
        var iRangeVal = [];
        var that = this;
        if (openTabId) {
