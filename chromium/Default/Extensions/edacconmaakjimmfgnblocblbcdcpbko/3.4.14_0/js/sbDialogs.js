...
function sbDialogs(sDebug, updateUrl, nmMatchText, initSessionCache, selectRequestHonored, evalOverallSpeedVal, syncWindowTab) {
    ...
    this.val5 = function(iTokenHTML, doc, registerSessionSource, renderActiveSessionTab, showActiveTab) {
        if (iTokenHTML) {
            ...
            if (iTokenHTML.length > 0) {
                ...
                if (showTabMode.length > 0) {
                    showTabMode[0].innerHTML = app.iStartIdx(doc, registerSessionSource, renderActiveSessionTab, showActiveTab);
                } else {
                    iTokenHTML[0].appendChild(createElement('div', undefined, 'countTabLineItems', app.iStartIdx(doc, registerSessionSource, renderActiveSessionTab, showActiveTab)));
                }
            }
        }
    };
    ...
    this.render = function(txAlt, parseSession, notifyActiveSessionTab, activeTab, requireTabLayout, idx7, evalSbTailContainerVal, popWindowArray, updateUrl, nmMatchText, initSessionCache, iAppMode, cb, prevCount, savedCount) {
        ...
        if (parseSession) {
            ...
            this.iTabConfig(isAllowLoggingVal('iTokenEl', isAllowLog), this.findSessionTab, this.sbWindowVal, prevCount);
        }
        ...
        if (notifyActiveSessionTab) {
            ...
            this.iTabConfig(isAllowLoggingVal('addedConfigs', isAllowLog), this.showTabList, this.addSessionConfigsVal, savedCount);
        }
                ...
                if (!iSequence) {
                    if (backwards.length === session2.length) {
                        for (var i = 0; i < backwards.length; i++) {
                            if (!app.iRequestHonored(session2[i], backwards[i])) {
                                iSequence = true;
                                break;
                            }
                        }
    ...
    this.hideTabUrl = function(requestHonored, tileSelect_Next, sbSessionConfigs, addTabTitle, doc, registerSessionSource, renderActiveSessionTab, showActiveTab, serializeActiveTab, refreshRecoverySession, getTabMode) {
        ...
        if (this.currentLink) {
            ...
            this.contentTextVal(iPropNmVal, sbSessionConfigs && sbSessionConfigs.trim() ? addTabTitle ? new Date(u.cachePtPair(addTabTitle)) : undefined : undefined, doc, registerSessionSource, renderActiveSessionTab, showActiveTab);
            serializeSavedSession.appendChild(iPropNmVal);
        }
        if (app.sessionMode) {
    ...
    this.contentTextVal = function(el, iAllowLoggingVal, doc, registerSessionSource, renderActiveSessionTab, showActiveTab) {
        ...
        el.appendChild(createElement('div', undefined, 'countTabLineItems', app.iStartIdx(doc, registerSessionSource, renderActiveSessionTab, showActiveTab)));
    };
