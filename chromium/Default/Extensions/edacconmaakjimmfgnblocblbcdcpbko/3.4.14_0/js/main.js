...
function initDOM() {
    ...
    iPredicateVal.SBDB.expectStatusVal('hidePanePositionUI', true, function(value) {
        if (value === 'true') {
            $('#wotLoc').hide();
        }
    });
        ...
        if (iPredicateVal && iPredicateVal.updateTabUrl) {
            ...
            iPredicateVal.SBDB.tabRow(function(toggleSessionTabMode) {
                iPredicateVal.SBDB.evalSessionConfigsToAddVal(function(isSBTabVal) {
                    handleSync = new selectSessionTab(isSBTabVal, toggleSessionTabMode, q('iSessionConfigHead'), q('refreshCurrentSession'), q('iNumberVal'), q('splitter'), q('evalSbGroupVal'), q('resetCurrentSessionCache'), q('evalOnlyCountDupesVal'), q('headerErrVal'), q('iMsg'), q('iFinalCssClsVal'), q('saveSession'));
                    app.idx2();
                            ...
                            iPredicateVal.SBDB.expectStatusVal('suppressOneTimeStartupMessageForLongTermUser', true, function(sortTabList) {
                                if (sortTabList === 'true') {
                                    iPredicateVal.SBDB.expectStatusVal('versionMessageReceived', true, function(value) {
                                        if (value != uvMessage) {
                                            if (toggleSessionTabMode !== 'right') {
                                                iPredicateVal.SBDB.resetIcon('hidePanePositionUI', 'true');
                                                $('#wotLoc').hide();
                                            }
                                            setTimeout(function() {
                                                maxCount('Have you backed up Session Buddy lately?<br><div><a id="backupNow" style="text-decoration:underline;">Back up now</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a id="imgVTag" style="text-decoration:underline;">Learn more</a></div>', 53, -1);
                                            }, 2e3);
                    ...
                    $('#runningVal').qtip({
                        content: {
                            text: 'Merge into single session'
                        },
                        position: {
                            my: 'bottom center',
                            at: 'top center',
...
function appMenuButton() {
    ...
    $('#finalCssClsVal').delegate('#sbRangeVal', 'click', function() {
        ...
        hashOriginator = true;
        window.open('http://sessionbuddy.com/backupRestore/backupRestore.html', '_blank');
...
function sortOriginWindows(w) {
    ...
    w.find('.bypassSessionMode').text(w.data('nx_title') || 'Window ' + w.data('wSeq')).scrollLeft(0).attr('contentEditable', false).qtip('enable');
}
...
function setPrimaryTabButton(w) {
    if (detOpt === 'saved' || detOpt === 'previous') {
        ...
        if (selectWindowTab[wSeq - 1].incognito = !selectWindowTab[wSeq - 1].incognito) {
            var el = createElement('div', undefined, 'iFirstInList');
            w.find('.bypassSessionMode').css('max-width', 'calc(100% - 62px)').css('margin-left', '2px');
            $(el).insertBefore(w.find('.bypassSessionMode')).qtip({
                content: {
            ...
            w.find('.iFirstInList').qtip('destroy', true).remove();
            w.find('.bypassSessionMode').css('max-width', '').css('margin-left', '');
        }
    ...
    chrome.extension.onMessage.addListener(function(request, sender, sendResponse) {
        if (!request.iBackwards && request.fctRefVal != tabIdentifier || request.iBackwards && request.iBackwards === tabIdentifier) {
            if (request.id === 'augmentWindowArray') {
                ...
                refreshSession(request.data.sActionPending);
                ...
                if (iSBDBVal === request.data.id) {
                    if (notifyWindowTab) {
                        ...
                        $('#processSessionState, #splitter > div').removeClass('newWindowProxy');
    ...
    chrome.tabs.getCurrent(function(t) {
        iWindow1Val(wins, placeLinkImage, t.windowId, t.id, false, function(doc, registerSessionSource, renderActiveSessionTab, showActiveTab) {
            ...
            if (detOpt != 'current') {
                ...
                q('iFinalCssClsVal').classList.remove('alsoSelectVal');
                $('#processSessionState, #splitter > div').addClass('newWindowProxy');
                detOpt = 'current';
            }
            if (cb) {
                cb();
...
function componentVal(requestHonored, sbTokenVal, cb, iTabCount) {
    ...
    iPredicateVal.SBDB.renderWindowTab(requestHonored, undefined, function(iSearchTerms) {
        if (iSearchTerms) {
            ...
            } : cb, iSearchTerms.thumbnail);
        }
    });
}

function resetSessionWindow(requestHonored, wins, formatActiveSessionTab, iRegisterValue1Val, cacheSessionRoot, evalReg2Val, sbTokenVal, cb, gid) {
    iWindow1Val(wins, requestActiveTab, false, false, false, function(doc, registerSessionSource, renderActiveSessionTab, showActiveTab) {
        if (sbTokenVal) {
        ...
        if (iSBDBVal != requestHonored || detOpt != 'saved' || formatActiveSessionTab == '' || formatActiveSessionTab && formatActiveSessionTab.trim().length > 0) {
            if (formatActiveSessionTab && formatActiveSessionTab.trim()) {
                ...
                foundTabCandidatesVal('Saved Session', 'Click to name this session');
                q('iFinalCssClsVal').classList.remove('alsoSelectVal');
                q('iSessionCountVal').value = '';
            }
            iSBDBVal = requestHonored;
...
function iNoText(wins, stringArray, includeSeqPropVal, sessionState, toggleDisplayedSession, evalOutMatchedTabUrlsVal, appMsg, augmentWindowCount, extractWindowTitle) {
    ...
    if (keyWins.length) {
        ...
        if (extractWindowTitle && includeSeqPropVal) {
            extractWindowTitle.windows = keyWins;
    ...
    if (s) {
        ...
        for (var i = 0; i < s.length; i++) {
            if (copyErrorDetail) {
                extractWindowTitle = {
                    type: 'saved',
                    name: s.item(i).name ? s.item(i).name : annotationClsVal === 'JSON' || annotationClsVal === 'JSON_All' ? undefined : 'Saved Session ' + copyActiveTab + ' [' + (app.dateDisplayType === 'standard' ? moment(s.item(i).modificationDateTime).format('L LT') : moment(s.item(i).modificationDateTime).format(app.customDateFormat)) + ']',
            ...
            if ((normalizeTitle = bImgHideURLs(u.severityVal(s.item(i).windows), stringArray, includeSeqPropVal, sessionState, toggleDisplayedSession, evalOutMatchedTabUrlsVal, appMsg, copyErrorDetail ? 0 : isSessionConfigSavedVal, extractWindowTitle)) && !s.item(i).name) {
                isSessionConfigSavedVal += normalizeTitle;
                copyActiveTab++;
            }
        }
...
function refreshSession(requestHonored) {
...
function windowStatus(addDisplayedSession, dupe, cb) {
    ...
    iPredicateVal.SBDB.addTabIcon(initValue + '' != newValue + '', dupe, newValue, cb);
}
                                                                ...
                                                                windowStatus(q('reloadWindowSource'), 'sessionSummaryRender_PanelPosition', function(currentSessionCountsNotifyVal) {
                                                                    ...
                                                                    windowStatus(q('evalSession1'), 'tabFiltering_FilterSessionBuddyTabs', function(currentSessionCountsNotifyVal) {
                                                                        ...
                                                                        windowStatus(q('evalRecoverySessionVal'), 'tabFiltering_FilterChromeAdministrativeTabs', function(currentSessionCountsNotifyVal) {
                                                                            ...
                                                                            adornment = adornment || currentSessionCountsNotifyVal !== false;
                                                                            if (cb) {
                                                                                cb(subArrayVal, addTabSource, showCurrentWindow, adornment, iPositionVal, createTabArray, rangeCumulativeVal, formatSessionWindow, evalSessionConfig2Val);
                                                                            }
                                                                        }, function(err) {
        ...
        app.currentSessionSrc(selectWindowTab, undefined, function(doc, registerSessionSource, renderActiveSessionTab, showActiveTab) {
            iPredicateVal.SBDB.evalRespVal(selectWindowTab, addAfterEl ? 'Copy of ' + addAfterEl : 'Copy of saved session', new Date().toJSON(), doc, registerSessionSource, renderActiveSessionTab, showActiveTab, function(requestHonored, lineitemElSelectedVal) {
                if (requestHonored === null) {
                    ...
                    initTab(requestHonored, 'saved');
                    txType(true, app.sbLinkVal('saved', requestHonored, undefined, undefined, addAfterEl ? 'Copy of ' + addAfterEl : 'Copy of saved session', lineitemElSelectedVal, doc, registerSessionSource, renderActiveSessionTab, showActiveTab));
                }
            });
        });
    }
    ...
    function syncWindowTitle(sAllowLogging, cb) {
        ...
        for (var i = 0; i < selectWindowTab.length; i++) {
            if (sbProp.contains(i)) {
                keyWins.push(selectWindowTab[i]);
            }
        }
        if (wDelete) {
            if (detOpt === 'current') {
                return placeHorizontal(wDelete, function() {
                    app.currentSessionSrc(keyWins, undefined, function(doc, registerSessionSource, renderActiveSessionTab, showActiveTab) {
                        iPredicateVal.SBDB.evalRespVal(keyWins, sAllowLogging, null, doc, registerSessionSource, renderActiveSessionTab, showActiveTab, function(requestHonored, lineitemElSelectedVal) {
                            if (requestHonored === null) {
                                maxCount('Failed to save session', 2);
                                return cb && cb();
                            }
...
function fctRef() {
    ...
    if (u.isNumeric(q('evalSessionConfig').value)) {
        u.enable(q('finalCssClsIdx'));
    } else {
        u.disable(q('finalCssClsIdx'));
    }
