...
ChromeExOAuth.prototype.signURL = function(url, method, opt_params) {
  ...
  //MODIFICATION
  /*
  if (window.chromeExOAuthConfig.tmp && window.chromeExOAuthConfig.tmp.tokenParamName) {
    params[window.chromeExOAuthConfig.tmp.tokenParamName] = token;
  }
  */
...
ChromeExOAuth.initCallbackPage = function() {
	...
	port.postMessage({
		id:			"101",
		method:	"GET",
		object:		"chromeExOAuthConfig"
	});
	callbackHandler["101"] = function() {
		var oauth_config = window.chromeExOAuthConfig;
		var oauth = ChromeExOAuth.fromConfig(oauth_config);
		//background_page.chromeExOAuthRedirectStarted = true;
		port.postMessage({
			id:			"102",
			method:	"SET",
			object:		"chromeExOAuthRedirectStarted",
			value:		true
		});
		callbackHandler["102"] = function() {
			oauth.initOAuthFlow(function (token, secret) {
				//background_page.chromeExOAuthOnAuthorize(token, secret);
				port.postMessage({
					id:			"103",
					method:	"EXEC",
					object:		"chromeExOAuthOnAuthorize",
					param: {
						token:	token,
						secret:	secret
					}
				});
				callbackHandler["103"] = function() {
  ...
  if (!this.hasToken()) {
    ...
    if (params['chromeexoauthcallback'] == 'true') {
      ...
      this.getRequestToken(function(url) {
		url += "&oauth_callback="+escape(window.location.href+"?chromeexoauthcallback=true");
...
ChromeExOAuth.prototype.getRequestToken = function(callback, opt_args) {
  ...
  if(this.app_name) params.xoauth_displayname = this.app_name;
  if(this.oauth_scope) {
    if(this.oauth_scope.substr(0,1) != "#") {
	  params.scope = this.oauth_scope;
	}
  }
...
ChromeExOAuth.prototype.getAccessToken = function(oauth_token, oauth_verifier,
  ...
  callbackHandler.thisObject = this;
  //var bg = window.backgroundPage;
  port.postMessage({
	id:			"201",
	method:	"GET",
	object:		"chromeExOAuthRequestingAccess"
  });
  callbackHandler["201"] = function() {
	  if (window.chromeExOAuthRequestingAccess == false) {
		...
		callbackHandler["202"] = function() {
			var result = OAuthSimple().sign({
			  path : callbackHandler.thisObject.url_access_token,
			  parameters: {
				"oauth_token" : oauth_token,
				"oauth_verifier" : oauth_verifier
			  },
			  signatures: {
				consumer_key : callbackHandler.thisObject.consumer_key,
				shared_secret : callbackHandler.thisObject.consumer_secret,
				oauth_secret : callbackHandler.thisObject.getTokenSecret(callbackHandler.thisObject.oauth_scope)
			  }
			});

			var onToken = ChromeExOAuth.bind(callbackHandler.thisObject.onAccessToken, callbackHandler.thisObject, callback);
			ChromeExOAuth.sendRequest("GET", result.signed_url, null, null, onToken);
		  }
		}
	}
