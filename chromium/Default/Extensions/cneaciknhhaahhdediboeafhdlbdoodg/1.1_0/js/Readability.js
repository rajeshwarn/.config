...
Readability.prototype = {
  ...
  REGEXPS: {
    unlikelyCandidates: /banner|combx|comment|community|disqus|extra|foot|menu|modal|related|remark|rss|share|shoutbox|sidebar|skyscraper|sponsor|ad-break|agegate|pagination|pager|popup|textAd/i,
    okMaybeItsACandidate: /and|body|column|main|shadow|textAd/i,
    positive: /article|body|content|entry|hentry|main|page|pagination|post|text|blog|story|ingredient/i,
    negative: /hidden|^hid$| hid$| hid |^hid |banner|combx|comment|com-|contact|foot|footer|footnote|masthead|meta|modal|outbrain|promo|related|scroll|share|shoutbox|sidebar|skyscraper|sponsor|shopping|tags|tool|widget|textAd/i,
    ...
    while (true) {
      ...
      **/
      ...
      if (topCandidate === null || topCandidate.tagName === "BODY") {
        ...
        this.log("a", parentOfTopCandidate);
  ...
  **/
  _removeScripts: function(doc) {
    for (var i = doc.getElementsByTagName("script").length - 1; i >= 0; i--) {
      doc.getElementsByTagName("script")[i].parentNode.removeChild(doc.getElementsByTagName("script")[i]);
    }
    for (var i = doc.getElementsByTagName("noscript").length - 1; i >= 0; i--) {
      doc.getElementsByTagName("noscript")[i].parentNode.removeChild(doc.getElementsByTagName("noscript")[i]);
    }
...
//          this._checkChildNodes(_node);
//        } else {
//          this._deleteElement(_node);
//        }
//      }
  },
  ...
  _checkChildNodes:function(node) {
    ...
    for(var i=0;i<childNodes.length;i++) {
      ...
      if (childNode.childNodes.length > 0) {
        this._checkChildNodes(childNode);
      } else {
        this._deleteElement(childNode);
      }
    }
  },

  _deleteElement:function(_node) {
    for (var j = 0, _j = this._skip_link_from_domain.length; j < _j; j++) {
          if (_node.href !== undefined && _node.href !== null && _node.href.indexOf(this._skip_link_from_domain[j]) > -1) {
            _node.parentNode.removeChild(_node);
        }
      }

      for (var k = 0, _k = this._skip_image_from_domain.length; k < _k; k++) {
          if (_node.src !== undefined && _node.src !== null && _node.src.indexOf(this._skip_image_from_domain[k]) > -1) {
              _node.parentNode.removeChild(_node);
          }
      }

      for (var l = 0, _l = this._keep_video_from_domain.length; l < _l; l++) {
          if (_node.src !== undefined && _node.src !== null && _node.src.indexOf(this._keep_video_from_domain[l]) > -1) {
              _node.parentNode.removeChild(_node);
          }
      }
  },
