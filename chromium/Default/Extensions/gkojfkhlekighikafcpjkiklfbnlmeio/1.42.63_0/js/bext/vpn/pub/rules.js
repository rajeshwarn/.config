...
define(['jquery', 'underscore', 'backbone', '/bext/pub/backbone.js',
    ...
    zutil, storage, date, be_info, be_premium, be_tabs, etask){
...
function clean_cookies(){
    ...
    {
        etask([function(){
            return etask.all([etask.cb_apply(B.cookies.get_all,
                [{name: 'DS'}]), etask.cb_apply(B.cookies.get_all,
                [{name: 'DE2'}])]);
        }, function(cookies_array){
            var cookies = cookies_array[0].concat(cookies_array[1]);
            // XXX pavlo: ask pavlo if you don't understand these urls below
