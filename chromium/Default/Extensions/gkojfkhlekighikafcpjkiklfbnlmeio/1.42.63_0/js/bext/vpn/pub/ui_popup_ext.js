...
E.init = function(){
    try {
        ...
        if (!E.no_be_commercial && !E.be_commercial)
        {
            B.backbone.client.ping('be_commercial', 50, function(ret){
                if (ret.error)
                    E.no_be_commercial = true;
                else
                {
                    E.be_commercial = B.backbone.client.start('be_commercial');
                    if (B.use_msg)
                    {
                        E.be_commercial.on('change:_backbone_client_started',
                            inited_cb);
                    }
                }
                return E.init();
...
E.uninit = function(){
    try {
        ...
        if (E.R)
        {
            ...
            if (E.be_commercial)
                B.backbone.client.stop('be_commercial');
...
function is_common_inited(){
    return E.R.get('_backbone_client_started') &&
        ...
        (E.no_be_commercial || E.be_commercial.get('_backbone_client_started'));
}
