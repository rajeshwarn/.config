...
function askIncludeFeedlyScript( partialURI, onComplete )
{
	...
	window.setTimeout( function(){
		try
		{
			chrome.runtime.sendMessage( "hipbfijinpcgfogaopmgehiegacbhmob",
					{},
					function( response )
					{
					    if( response != null && response.alive == true )
					    {
							$feedly( "[ext] no need to inject feedly mini content script. app is installed." );
					    }
					    else
					    {
							$feedly( "[ext] loading feedly mini as an extension." );
							loadFeedlyMini();
					    }
					}
					);
...
function askIncludeLibraries()
{
	...
	include ( "js/10101_gc-mini_5642.js");
